...
export const updateBofMaharashtra = async (req, res) => {
    try {
        ...
        const updatedForm = await BofMaharastraModel.findOneAndUpdate(
            { uniqueId: String(id) },
            { $set: updateData },
...
export const requestReworkBofMaharashtra = async (req, res) => {
    try {
        ...
        // Validate ID
        if (!id || typeof id !== 'string') {
            console.error("[requestReworkBofMaharashtra] Invalid ID format:", id);
            return res.status(400).json({
                success: false,
                message: "Invalid ID format"
            });
        ...
        console.log("[requestReworkBofMaharashtra] Request received:", {
            id,
            username,
            userRole,
            clientId: clientId ? clientId.substring(0, 8) + "..." : "missing"
        ...
        // Verify manager/admin role
        if (req.user.role !== "manager" && req.user.role !== "admin") {
            console.error("[requestReworkBofMaharashtra] Unauthorized role:", req.user.role);
            return res.status(403).json({
                success: false,
        ...
        try {
            ...
            console.error("[requestReworkBofMaharashtra] Database query error:", dbError.message);
            return res.status(400).json({
                success: false,
                message: "Invalid request parameters"
            });
        ...
        if (!form) {
            console.error("[requestReworkBofMaharashtra] Form not found:", id);
            return res.status(404).json({
                success: false,
                message: "BOF Maharashtra form not found"
            });
        ...
        // CLIENT ISOLATION - CRITICAL
        if (form.clientId !== clientId) {
            console.error("[requestReworkBofMaharashtra] Client isolation violation:", {
                recordClient: form.clientId,
                requestClient: clientId
            });
            return res.status(403).json({
        ...
        console.log("[requestReworkBofMaharashtra] Requesting rework");
        ...
        if (!updatedForm) {
            console.error("[requestReworkBofMaharashtra] Failed to update form:", id);
            return res.status(500).json({
                success: false,
                message: "Failed to request rework for BOF Maharashtra form"
            });
        }
        ...
        console.error("[requestReworkBofMaharashtra] Unexpected error:", error);
        res.status(500).json({
            success: false,
