...
import { getAllBofMaharashtra } from "../services/bomFlatService";
...
import { getFormRouteForBank } from "../config/bankFormMapping";
...
const DashboardPage = ({ user, onLogout, onLogin }) => {
    ...
    // Helper function to navigate to the correct form based on bank name
    const navigateToEditForm = (record) => {
        ...
        // Get the form route dynamically based on bank configuration
        const formRoute = getFormRouteForBank(record?.bankName);
        
        console.log("✅ Routing to form:", formRoute, "for bank:", record?.bankName);
    ...
    const fetchFiles = async (isInitial = false, showLoadingIndicator = true) => {
        try {
            ...
            const response = {
                ...valuationsResponse,
                data: [
                    ...(Array.isArray(valuationsResponse?.data) ? valuationsResponse.data : []),
                    ...(Array.isArray(bofResponse?.data) ? bofResponse.data : [])
                ]
            };
                                        ...
                                        <TableBody>
                                            {paginatedFiles.map((record) => (
                                                <TableRow key={record._id} className="hover:bg-neutral-100 border-b border-neutral-200 transition-colors duration-200">
                                                    ...
                                                    <TableCell className="text-xs px-1 py-2 truncate font-semibold text-neutral-700">{record.bankName}</TableCell>
